...
import importlib
# Use the correct module name based on the actual file in 01_SRC
providers = importlib.import_module("providers")
stream = providers.stream
TextChunk = providers.TextChunk
AssistantTurn = getattr(providers, "AssistantTurn", None)
    ...
    try:
        ...
        if os.environ.get("MOCK_BENCHMARK") == "1":
            ...
            for event in stream(model_name, system, messages, [], config):
                ...
                elif AssistantTurn and isinstance(event, AssistantTurn):
...
def main():
    ...
    if results:
        dest = Path("03_DOCS/2026-07-02_CLAW_OLLAMA_BENCHMARKS_V01.json")
