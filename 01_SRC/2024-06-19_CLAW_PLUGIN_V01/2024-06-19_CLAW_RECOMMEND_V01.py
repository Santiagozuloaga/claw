...
import importlib; _m_types = importlib.import_module(".2024-06-19_CLAW_TYPES_V01", __package__); globals().update({'PluginManifest': getattr(_m_types, 'PluginManifest'), 'PluginScope': getattr(_m_types, 'PluginScope')})
import importlib; _m_store = importlib.import_module(".2024-06-19_CLAW_STORE_V01", __package__); globals().update({'list_plugins': getattr(_m_store, 'list_plugins'), 'USER_PLUGIN_DIR': getattr(_m_store, 'USER_PLUGIN_DIR')})
