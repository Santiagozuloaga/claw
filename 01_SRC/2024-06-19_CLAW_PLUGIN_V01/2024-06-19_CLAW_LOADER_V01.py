...
import importlib; _m_store = importlib.import_module(".2024-06-19_CLAW_STORE_V01", __package__); globals().update({'list_plugins': getattr(_m_store, 'list_plugins')})
import importlib; _m_types = importlib.import_module(".2024-06-19_CLAW_TYPES_V01", __package__); globals().update({'PluginEntry': getattr(_m_types, 'PluginEntry'), 'PluginScope': getattr(_m_types, 'PluginScope')})
...
def register_plugin_tools(scope: PluginScope | None = None) -> int:
    ...
    globals().update({'register_tool': getattr(m_2024_06_19_CLAW_TOOL_REGISTRY_V01, 'register_tool'), 'ToolDef': getattr(m_2024_06_19_CLAW_TOOL_REGISTRY_V01, 'ToolDef')})
    count = 0
    for entry in load_all_plugins(scope):
        if not entry.manifest or not entry.manifest.tools:
            continue
