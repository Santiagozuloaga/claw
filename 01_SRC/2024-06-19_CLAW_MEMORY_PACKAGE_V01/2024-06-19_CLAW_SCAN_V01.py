...
import importlib; _m_store = importlib.import_module(".2024-06-19_CLAW_STORE_V01", __package__); globals().update({'get_memory_dir': getattr(_m_store, 'get_memory_dir'), 'parse_frontmatter': getattr(_m_store, 'parse_frontmatter'), 'INDEX_FILENAME': getattr(_m_store, 'INDEX_FILENAME')})
