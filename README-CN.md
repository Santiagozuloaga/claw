...
| [claw-code](#二-claw-code) | Python | 清室架构重写 | 109 个 |
| [ClawSpring](#三-clawspring) | Python | 极简多模型重实现 (v3.05.5) | ~120 个 |
...
一个快速、易用的个人 AI 助手，支持任何模型，受 OpenClaw 和 Claude Code 启发。与 `claw-code`（专注于架构映射）不同，ClawSpring 是一个可以立即使用的真实编码助手。它支持 20 多个闭源模型和本地开源模型，并已发展成为具有多代理编排、持久记忆和技能系统的功能丰富的 v3.05.5。

### 特性

- **多模型支持**：支持 Anthropic, OpenAI, Gemini, DeepSeek, 以及通过 Ollama 运行的本地模型。
- **工具系统**：内置 25 个工具，包括文件读写、代码搜索、终端执行、网页抓取等。
- **多代理协作**：支持生成特定类型的子代理（如 coder, reviewer, researcher）并行处理任务。
- **持久记忆**：支持用户和项目两个级别的持久化记忆，具备 AI 搜索和冲突检测功能。
- **技能系统**：支持通过 Markdown 定义的可重用技能，支持参数替换和分支执行。
- **SSJ 开发者模式**：提供强大的交互式菜单，涵盖头脑风暴、TODO 管理、自动执行、专家辩论等工作流。
- **Telegram 桥接**：支持通过 Telegram 机器人远程控制助手。
- **语音输入**：支持本地离线语音识别（Whisper）。
